...
const AdminDashboard = ({ students, onNavigate, onLogout, onRefresh }) => {
  ...
  const labs = [
    { lab: 1, titleEn: "HTML Basics" },
    { lab: 2, titleEn: "HTML Forms & Tables" },
    { lab: 3, titleEn: "CSS Fundamentals" },
    { lab: 4, titleEn: "CSS Layout & Flexbox" },
    { lab: 5, titleEn: "CSS Grid & Responsive" },
    { lab: 6, titleEn: "JavaScript Basics" },
    { lab: 7, titleEn: "JavaScript DOM" },
    { lab: 8, titleEn: "JavaScript Events" },
    { lab: 9, titleEn: "JavaScript Forms" },
    { lab: 10, titleEn: "Final Project" },
  ];
  ...
  const StudentDetailModal = ({ student, onClose }) => {
    const completedLabs = student.progress 
      ? Object.values(student.progress).filter(p => p.completed).length
      : 0;
    const progressPercent = Math.round((completedLabs / totalLabs) * 100);
    ...
    return (
      <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
        <div className="bg-slate-800 rounded-2xl border border-slate-700 max-w-3xl w-full max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
          ...
          <div className="p-6">
            <h3 className="text-lg font-semibold text-white mb-4">Lab Progress</h3>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {labs.map(lab => {
                const labProgress = student.progress?.[`lab${lab.lab}`];
                const isCompleted = labProgress?.completed;
                const score = labProgress?.score || 0;
                const attempts = labProgress?.attempts || 0;

                return (
                  <div key={lab.lab} className={`p-3 rounded-lg border ${isCompleted ? 'bg-emerald-900/20 border-emerald-500/30' : 'bg-slate-700/30 border-slate-600'}`}>
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-blue-400 text-xs font-semibold">Lab {lab.lab}</span>
                      {isCompleted && <span className={`font-bold text-sm ${getScoreColor(score)}`}>{score}%</span>}
                    </div>
                    <p className="text-white text-xs truncate">{lab.titleEn}</p>
                    {isCompleted ? (
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-blue-900/30">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-blue-300/70 text-xs">Web Systems Course | لوحة تحكم المدير</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1 relative">
              ...
              <input
                ...
                className="w-full bg-slate-700/50 border border-slate-600 rounded-lg pl-10 pr-4 py-2.5 text-white placeholder-slate-400 focus:outline-none focus:border-blue-500/50"
              />
            </div>
            ...
            <select
              ...
              className="bg-slate-700/50 border border-slate-600 rounded-lg px-4 py-2.5 text-white focus:outline-none focus:border-blue-500/50"
            >
        ...
        <div className="bg-slate-800/50 rounded-2xl border border-slate-700 overflow-hidden">
          ...
          {filteredStudents.length === 0 ? (
            <div className="p-12 text-center">
              ...
              <button onClick={handleRefresh} className="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-lg text-sm transition-colors">
                  ...
                  {filteredStudents.map(student => {
                    ...
                    return (
                      <tr key={student.id || student.studentNumber} className="hover:bg-slate-700/30 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <span className="px-2 py-1 bg-slate-700 rounded text-blue-400 text-sm">{student.sectionNumber}</span>
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex items-center justify-center gap-2">
                            <div className="w-24 h-2 bg-slate-700 rounded-full overflow-hidden">
                              <div className="h-full bg-gradient-to-r from-blue-500 to-blue-400 transition-all" style={{ width: `${progressPercent}%` }} />
                            </div>
                            <span className="text-slate-400 text-sm">{completedLabs}/{totalLabs}</span>
                          </div>
                        </td>
                        ...
                        <td className="px-4 py-3 text-right">
                          <button onClick={() => setSelectedStudent(student)} className="px-3 py-1.5 bg-blue-600/20 hover:bg-blue-600/30 text-blue-400 rounded-lg text-sm transition-colors">
                            View Details
                          </button>
                        </td>
                      </tr>
